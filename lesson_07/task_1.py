...
# Здесь пишем код
class Segment:
    """Используется для создания 'отрезков', представленных координатами начала и конца отрезка"""
    ...
    def length(self) -> float:
        ...
        length = ((self.point_b[0] - self.point_a[0]) ** 2 + (self.point_b[1] - self.point_a[1]) ** 2) ** 0.5
        return round(length, 2)

    def x_axis_intersection(self) -> bool:
        """Определяем пересекает ли отрезок ось X"""

        return self.point_a[1] * self.point_b[1] <= 0

    def y_axis_intersection(self) -> bool:
        """Определяем пересекает ли отрезок ось Y"""

        return self.point_a[0] * self.point_b[0] <= 0
